...
description: Verify Clix Mobile is PRODUCTION READY end-to-end. Triggers like "check production readiness", "are we ready to ship", "readiness check", "is clix-mobile live", "is the app published". Runs a script that confirms both Google Play and F-Droid (self-hosted + mainline) are PUBLISHED and the app/site are healthy.
category: release
version: 1.1.0
---

# Clix Mobile — Readiness Check

Single command that answers one question: **is Clix Mobile production ready?**

"Production ready" has a precise definition of done:

- The app is **PUBLISHED on Google Play** (`com.clix.mobile`).
- The app is **PUBLISHED on F-Droid mainline** (`f-droid.org/packages/com.clix.mobile`).
...
- **B. F-Droid self-hosted repo LIVE** (REQUIRED)
  - `https://bolajiev.github.io/clix-mobile/fdroid/repo/index-v1.json` parses and contains `com.clix.mobile`; prints served versionName.
  - `https://github.com/bolajiev/clix-mobile/releases/latest` returns 200/3xx.
- **C. F-Droid MAINLINE published** (REQUIRED, headline)
  - `https://f-droid.org/packages/com.clix.mobile/` returns 200. A 404 means not yet merged on f-droid.org → this gate FAILS. Do not confuse with the self-hosted gate (B).
- **D. Google Play PUBLISHED** (REQUIRED, headline)
  - `https://play.google.com/store/apps/details?id=com.clix.mobile` returns 200 with a real store page. While in-review/draft it 404s → gate FAILS.
- **E. Web presence** (REQUIRED)
  - landing `/`, `/guide/`, `/privacy/`, `sitemap.xml`, `robots.txt`, `og.png`, `fdroid-qr.png`, `apk-qr.png` under `https://bolajiev.github.io/clix-mobile/` all return 200.
- **F. Repo discoverability** (nice-to-have, WARN only)
  - `gh repo view bolajiev/clix-mobile --json repositoryTopics,homepageUrl` shows topics + homepage. Skipped gracefully if `gh` is missing/unauthenticated.
