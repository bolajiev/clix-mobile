...
**Known gated steps:**
- Play Console **API-access grant** to the service account — *was* the blocker; now granted (publish run 26662900471 succeeded, AAB on internal track, versionCode 19). One-time path if re-needed: Play Console → Setup → API access → link GCP project `clix-mobile-deploy` → grant `playstore-deploy@clix-mobile-deploy.iam.gserviceaccount.com` Admin/Release-manager.
...
**Repo**: `bolajiev/clix-mobile` (standalone, not part of opencode monorepo)
**Package name**: `com.clix.mobile`
...
python3 scripts/android-cua-smoke.py --e2e \
  ...
  --e2e-project-dir ~/workspace/clix-mobile \
...
python3 scripts/android-cua-smoke.py \
  ...
  --query "Open android app. Setup against remote opencode server. Go to sessions. \
           Open a new project inside ~/workspace/clix-mobile. \
...
Secrets required: `AZURE_OPENAI_API_KEY`, `AZURE_OPENAI_ENDPOINT` (already set on `bolajiev/clix-mobile`).
...
All project secrets are stored in Bitwarden vault under folder **`clix-mobile`**.
...
FOLDER_ID=$(bw list folders --session "$BW_SESSION" | jq -r '.[] | select(.name=="clix-mobile") | .id')
...
- `SENTRY_PROJECT` — `clix-mobile`

These same secrets are set as GitHub Actions secrets on `bolajiev/clix-mobile` for CI builds.
...
For pushes/gh CLI on this repo: `source ~/.env.d/github-bolajiev.env`

## Google Account

**ALL Google operations** (Play Console, GCP Console, YouTube, Firebase, etc.) use **`YOUR_GOOGLE_ACCOUNT_EMAIL`** — the YOUR_ORG_NAME account. `bolajievv@gmail.com` is the personal account and does NOT have access to the project GCP resources, Play Console, or YouTube channel. Always verify the active account in the top-right corner before making changes. If GCP console shows `bolajievv@gmail.com`, switch accounts via the avatar menu.

## Google Play Console

- **Developer account**: YOUR_ORG_NAME (ID: `8842655543970815326`), Google login `YOUR_GOOGLE_ACCOUNT_EMAIL`. The `/u/N/` index is NOT stable — if a console URL bounces to accept-terms/create-developer-account you're on the wrong Google account (e.g. `bolajievv@gmail.com` hits a ToS gate); use the developer-account chooser to reach VIBE.
- **Rebrand (2026-05-30)**: package renamed `com.clix.mobile` → `com.clix.mobile`. **App IS live on Play Store internal track** (v0.4.6, versionCode 33). CI publishes updates automatically via the service account after the first manual upload was completed. CI `packageName` = `com.clix.mobile`. NOTE: CI publish build needs the "Purge stale generated sources" step (commit 67e4c1f) or cached old-package autolinking breaks compile.
- **Legacy app (orphaned)**: `com.clix.mobile`, app ID `4975545755653045321` — published v19 to internal track (run 26662900471), superseded by the rebrand.
- **Track**: Internal testing (no review required, up to 100 testers)
- **Service account**: `playstore-deploy@clix-mobile-deploy.iam.gserviceaccount.com` (account-level API access already granted)

## Related Issues

- Upstream: `anomalyco/opencode#10288`
- Branch on upstream fork: `feat/android-backbone-10288` on `bolajiev/opencode`

## Clix Production Server (this box)

- **VPS**: Contabo (vmi3316871.contaboserver.net, `167.86.91.92`, Karlsruhe DE). Not a tailnet member — use nginx + CF, not Tailscale IPs.
- **opencode server**: systemd unit `opencode-serve` (restarts automatically), bound `127.0.0.1:4096`. Logs: `journalctl -u opencode-serve`.
- **Public URL for the Clix app**: `https://opencode.bolajiev.com` → nginx (`/etc/nginx/sites-available/opencode`) with basic auth → 127.0.0.1:4096. SSE-friendly (proxy_buffering off). TLS via certbot once DNS record exists. **LIVE** (2026-08-12): DNS record created (proxied), Let's Encrypt cert valid, verified end-to-end incl. SSE.
- **App credentials**: users `clix` and `opencode` (same password) in htpasswd `/etc/nginx/htpasswd/clix`; password in `~/.env.d/cloudflare.env` (`CLIX_OPENCODE_USER/PASS`). Connection in app: **Advanced mode** — URL `https://opencode.bolajiev.com` with **NO port** (quick mode appends :4096 which is not exposed), username `clix`, password. Quick mode works with `https://opencode.bolajiev.com:443` in the IP field (username defaults to `opencode`).
- **Cloudflare creds** (account `699cda20eb7d3d13644173c0b92ddad4`, zone `bolajiev.com` id `78255e91ba16a462efa81190ade4f17a`): `~/.env.d/cloudflare.env` (API token is READ-ONLY for DNS — write ops need dashboard or a DNS:Edit token).
- **DNS**: `bolajiev.com` on Cloudflare. Record: `opencode` A → `167.86.91.92` **proxied** (Cloudflare IPs `188.114.96.x/97.x`) — certbot HTTP-01 works through the proxy because nginx serves the ACME challenge on port 80.
- **R2 bucket**: endpoint `https://699cda20eb7d3d13644173c0b92ddad4.r2.cloudflarestorage.com` with Access/Secret keys in same env file.

## Terminal ↔ App sync (IMPORTANT)

The mobile app talks to the systemd `opencode-serve` backend on 127.0.0.1:4096.
A standalone `opencode` TUI runs its OWN in-process server — it never sees the
app's messages (and vice versa). For one live conversation across phone + terminal:

- Start terminal sessions with: `opencode attach http://127.0.0.1:4096 [-s <sessionID>]`
- Then both ends share the SAME server + SSE: messages appear live both ways.
- Non-interactive runs should use `opencode run --attach http://127.0.0.1:4096 ...`
  (explain-image.sh already does).
- The app also polls the open session every 15s as a fallback.
