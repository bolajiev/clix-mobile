# Clix Mobile — Handoff (2026-06-21)

App: **`com.clix.mobile`** (Clix Mobile) — Expo/React Native Android client
for a user self-hosted opencode AI server. Owner: YOUR_ORG_NAME.
Repo: `bolajiev/clix-mobile`. Full running log: `context.md`.
...
| #5 Website live | ✅ `https://opencode.bolajiev.com` live (Vercel, SSL) |
...
### Website
- `https://opencode.bolajiev.com` live — Vercel project `clix-mobile-site`
- DNS: A + TXT records added in Spaceship for `bolajiev.com`
...
1. **HN Show HN** — Tuesday–Thursday 8–10am ET. Template in `clix-mobile-demo-to-store` skill.
...
Add diff viewer + tool-call approval screenshots. Upload via Play Developer API (see `clix-mobile-demo-to-store` skill Phase 3).

---

## App URLs

- **Direct APK (works now):** https://github.com/bolajiev/clix-mobile/releases/latest
- **F-Droid self-hosted repo:** https://bolajiev.github.io/clix-mobile/fdroid/repo
  (add this URL in any F-Droid client). **LIVE @ v0.4.3** — verified 2026-06-02.
- **Google Play (NOT public yet):** https://play.google.com/store/apps/details?id=com.clix.mobile
  — 404s until production rollout; internal-testing track has v0.4.3.
- **F-Droid mainline (pending MR #39530):** https://f-droid.org/packages/com.clix.mobile/
...
- `67773fc` corrected IzzyOnDroid doc (was stale `com.clix.mobile`).
...
0. **Push the local commits (push is permission-gated for the agent — harness blocks `git push`).**
   **17 commits unpushed on `main`** as of 2026-06-08 (run `git log origin/main..main --oneline`
   ...
   Run (owner):
   ```
   cd /Users/engineer/workspace/clix-mobile && source ~/.env.d/github-bolajiev.env
...
1. **Privacy policy is live (Play-production blocker cleared).** Canonical privacy URL is
   now `https://bolajiev.github.io/clix-mobile/privacy/` — live & verified (HTTP 200) on
...
2. **F-Droid reach (any/all):**
   ...
   - **Mainline F-Droid**: MR #39530 is filed and open at
     ...
     `metadata/com.clix.mobile.yml` on the MR's source branch (local source:
...
3. **Growth → 1k downloads:** post the launch kit in `distribution/launch/`
   ...
   **Gate:** every post file contains `{{PLAY_URL}}` / `{{FDROID_URL}}` placeholders —
   ...
   `{{FDROID_URL}}` → `https://bolajiev.github.io/clix-mobile/fdroid/repo` (live now);
   `{{PLAY_URL}}` → `https://play.google.com/store/apps/details?id=com.clix.mobile`
...
adb install -r ~/workspace/clix-mobile/android/app/build/outputs/apk/release/app-release.apk
adb shell am start -n com.clix.mobile/.MainActivity
```
...
# Is the live F-Droid repo updated to com.clix.mobile @ v0.4.3?
curl -s https://bolajiev.github.io/clix-mobile/fdroid/repo/index-v1.json | \
...
- GitHub auth (local): `source ~/.env.d/github-bolajiev.env`.
