# Google Play Console Setup — Clix Mobile
...
- [ ] App created in Play Console (`com.clix.mobile`)
...
4. Description: `Automated Play Store deployment for com.clix.mobile`
...
2. All apps → (select `com.clix.mobile`)
3. Setup → API access
4. "Link a Google Cloud project" → select `clix-mobile-deploy`
5. "Link"
...
```bash
# Build AAB locally (in the clix-mobile repo)
cd android
...
cat /path/to/service-account.json | \
  gh secret set PLAY_STORE_SERVICE_ACCOUNT_JSON --repo bolajiev/clix-mobile
```
...
```bash
gh workflow run publish-play-store.yml --repo bolajiev/clix-mobile
```
