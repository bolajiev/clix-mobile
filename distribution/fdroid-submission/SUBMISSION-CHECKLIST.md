...
https://gitlab.com/fdroid/fdroiddata to add `com.clix.mobile` to the
...
git fetch upstream
git checkout -b add-com.clix.mobile upstream/master
```
...
```bash
cp /path/to/clix-mobile/distribution/fdroid-submission/metadata.yml \
   metadata/com.clix.mobile.yml
```

Verify:
- `metadata/com.clix.mobile.yml` exists
...
fdroid build com.clix.mobile:<versionCode>
```

If the build fails, fix `metadata/com.clix.mobile.yml` before filing the MR.
...
```bash
git add metadata/com.clix.mobile.yml
git commit -m "Add com.clix.mobile (Clix Mobile)"
git push origin add-com.clix.mobile
```

Go to https://gitlab.com/<YOUR_GITLAB_USERNAME>/fdroiddata → open an MR
against `fdroid/fdroiddata:master`.

MR title: `Add com.clix.mobile`
...
Clix Mobile is a free, open-source mobile client for the opencode AI
...
- Add `com.clix.mobile` to F-Droid's inclusion notice in `docs/fdroid.md`.
