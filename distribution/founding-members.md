...
**Goal:** Clix Mobile's first dollar. Current MRR: **$0**. No Stripe product exists yet.
...
> **Den — add a Bitwarden Secure Note named `STRIPE_TEST_SECRET_KEY` (folder `clix-mobile`) containing the `sk_test_...` key from https://dashboard.stripe.com/test/apikeys; the vault only has `OpenClawBot - STRIPE_SECRET_KEY` which is `sk_live_` and is off-limits.**

Why it's founder-gated: creating the test key requires an interactive login to the Stripe Dashboard on the
YOUR_ORG_NAME account (`YOUR_GOOGLE_ACCOUNT_EMAIL`). Stripe's login is anti-automation and the
...
| Item | State |
|---|---|
| `STRIPE_TEST_SECRET_KEY` in Bitwarden (`clix-mobile`) | **MISSING — founder-gated** |
...
> **[HOOK]**
>
> I'm the maintainer of Clix Mobile. I'm opening a small **Founding Member** group — 8 people —
...
| `SDBdevelopment` | You forked Clix Mobile last week — if you're deploying this for a team, I want to talk before I set pricing. |
