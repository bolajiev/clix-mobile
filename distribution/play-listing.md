# Google Play Store Listing — Clix Mobile

Copy-paste reference for the Play Console store listing for `com.clix.mobile`.
...
> https://play.google.com/store/apps/details?id=com.clix.mobile (1K+ installs;
...
```
Clix Mobile is the AI coding agent companion for developers who self-host opencode. Connect your Android phone to your opencode server and run AI-powered coding sessions from anywhere — review file diffs, approve tool calls, and guide Claude, GPT, Gemini, or any other model you configure, in real time.

<b>WHAT IT DOES</b>
Clix Mobile is a thin client for the opencode CLI (github.com/sst/opencode). It speaks the opencode HTTP + SSE API, so every AI coding session on your workstation or server is instantly reachable from your phone. All AI model calls go through your own server — your API keys, your code, your infrastructure. Nothing passes through our servers.
...
<b>OPEN SOURCE</b>
Clix Mobile is MIT licensed. Source code, issue tracker, and community at github.com/bolajiev/clix-mobile. Contributions welcome.

<b>PRIVACY</b>
Clix Mobile does not collect your code, prompts, or AI responses. All traffic goes directly from the app to YOUR opencode server — never through our infrastructure. With your opt-in consent we use Sentry for crash diagnostics and PostHog for anonymous usage analytics (no PII, no message content, off by default). Diagnostic reports you share are also delivered to our support inbox. If you choose to join the optional OpenCode Connect waitlist, we collect the email address you enter to notify you at launch.
...
| Email | support@bolajiev.com | Already verified during signup. |
| Phone | +1 360-504-8967 | Optional public; matches developer profile. |
| Website | https://opencode.bolajiev.com | Already verified. |
...
Suggested path: `https://bolajiev.github.io/clix-mobile/privacy/`
...
- Third-party SDKs (Sentry — https://sentry.io/privacy/; PostHog — https://posthog.com/privacy; **Brevo — https://www.brevo.com/legal/privacypolicy/, used only for the optional waitlist**) and our own self-hosted Chatwoot support inbox (support.chatwoot.example.com — not a third-party vendor)
...
Operator: YOUR_ORG_NAME, 519 S Henderson St, Seattle WA 98108-4522 USA
...
Via our own Chatwoot support inbox (support.chatwoot.example.com), only when you tap
"Share Report":
...
(support.chatwoot.example.com, retained until the conversation is resolved and
...
- Chatwoot support inbox (support.chatwoot.example.com) — we operate this
...
User rights:
- Email support@bolajiev.com to request deletion of crash records, analytics
...
Google requires this before publishing. Answers for Clix Mobile current state.
...
| Do you provide a way for users to request that their data is deleted? | Yes — via support@bolajiev.com |
...
```
Clix Mobile requires the user to bring their own opencode server (https://opencode.ai). To review the app:
...
If reviewers cannot self-host, contact support@bolajiev.com and we will provide a temporary hosted opencode endpoint for review.
```
...
- [x] Privacy policy live at https://bolajiev.github.io/clix-mobile/privacy/
