# Clix Mobile — Distribution + Monetization Strategy
...
|---|---|---|---|---|---|
| 1 | **Google Play (Internal)** | `com.clix.mobile` | ⏸ blocked on identity verification | Days after ID approved | CI ready. Track: `internal` first, then closed testing (12+ testers / 14d) before production. |
| 2 | **IzzyOnDroid** | `com.clix.mobile` (same key) | ❌ not started | 1–3 days | Submit prebuilt APK to https://codeberg.org/IzzyOnDroid/repodata/issues. Fastest OSS channel. |
| 3 | **F-Droid mainline** | `com.clix.mobile` (same key, reproducible build) | ❌ not started | 4–12 weeks | File MR at https://gitlab.com/fdroid/fdroiddata. Requires `expo-notifications` FCM audit + Sentry opt-in gate. |
| 4 | **Apple App Store** | `com.clix.mobile` | ⏸ pending iOS prep agent | Weeks (Apple enrollment $99 + review) | iOS agent running — separate report. |

**All channels: same package id (`com.clix.mobile`), same signing key.** Lets users update across stores in-place.
...
4. **Privacy policy URL** — live & verified at https://bolajiev.github.io/clix-mobile/privacy/ (this GitHub Pages URL is the canonical privacy URL used for store submissions; if the owner later deploys the branded `bolajiev.com/opencode/privacy`, it can replace it — reversible). Template in `play-listing.md`.
...
11. **GitHub Sponsors profile** — YOUR_ORG_NAME org. Tiers: $5 / $15 / $50.
...
| GCP project `clix-mobile-deploy` | ✅ Created |
...
| Website verification | ✅ bolajiev.com verified via Search Console auto-detection |
| Contact email verification | ✅ support@bolajiev.com verified via Play Console code |
...
2. (manual) Setup → API access → Link `clix-mobile-deploy`. Grant `playstore-deploy@…` "Release to production".
3. (manual) Create app `com.clix.mobile`. Fill listing from `play-listing.md`. Upload icon + feature graphic + screenshots. Complete Data safety + Content rating + App access.
...
2. File issue at https://codeberg.org/IzzyOnDroid/repodata/issues:
   ```
   App name: Clix Mobile
   Package: com.clix.mobile
   License: MIT
   GitHub release: https://github.com/bolajiev/clix-mobile/releases
   APK SHA-256: <sha256>
...
2. Create `metadata/com.clix.mobile.yml` with reproducible-build config (template in F-Droid report).
...
For email notification when Google approves identity: Google sends to YOUR_GOOGLE_ACCOUNT_EMAIL. Add a `gws gmail` poll script that watches for "Developer account verification" subject from `noreply@google.com` and pings via webhook / posts a GitHub issue.
