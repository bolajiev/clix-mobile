# bolajiev.com/opencode — Deploy Guide

## Status

- Vercel project: `clix-mobile-site` (team `bolajievs-projects`)
- Auto-domain (live now): https://clix-mobile-site.vercel.app
- Custom domain (pending DNS): https://opencode.bolajiev.com
...
**Log in to Cloudflare → bolajiev.com zone → DNS → Add record:**
...
```bash
curl -sI https://opencode.bolajiev.com/ | head -5
```
...
The `/privacy` page reads `../../../clix-mobile/distribution/privacy-policy.html`
...
```bash
curl -sI https://bolajiev.github.io/clix-mobile/privacy/ | head -3
```
