# Google Play Store — clix-mobile

Operational doc for shipping `com.clix.mobile` to Google Play under YOUR_ORG_NAME.
...
| Field | Value |
|---|---|
| Google account (owner) | YOUR_GOOGLE_ACCOUNT_EMAIL |
| Developer Account ID | **8842655543970815326** |
| Account type | Organization — YOUR_ORG_NAME |
| Developer name (public) | `YOUR_ORG_NAME` |
| D-U-N-S | 142059652 |
...
| Website ownership | ✅ verified — https://bolajiev.com/ (Search Console auto-detected meta tag) |
| Contact email | ✅ support@bolajiev.com verified |
...
| Resource | Value |
|---|---|
| Project | `clix-mobile-deploy` |
| Service account | `playstore-deploy@clix-mobile-deploy.iam.gserviceaccount.com` |
...
1. ✅ gcloud authed as `YOUR_GOOGLE_ACCOUNT_EMAIL`
...
1. (manual) Setup → API access → Link `clix-mobile-deploy`. Grant `playstore-deploy@…` "Release to production, exclude devices, and use Play App Signing".
2. (manual) Create app `com.clix.mobile`. Fill listing from `distribution/play-listing.md`.
...
Clix Mobile is also distributed via F-Droid (mainline) and IzzyOnDroid —
the two primary OSS Android app stores for privacy-conscious users.

All three channels use the **same signing key and same package id** (`com.clix.mobile`),
...
- Original handoff doc: `clix-mobile.playstore.md` (root, mostly historical)
