# Clix Mobile — Product Requirements Document (PRD)
...
All secrets are stored in Bitwarden under folder `clix-mobile`. See `AGENTS.md` for retrieval commands.
...
### 5.5 Observability
- **Sentry dashboard:** `sentry.io → vibetechnologies → clix-mobile`
- **Alert email:** configure issue alert rules on the Sentry project to send to `bolajievv@gmail.com` (or your distribution list).
...
- [ ] `https://opencode.bolajiev.com` serves the approved screenshots from
