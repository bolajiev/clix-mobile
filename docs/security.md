...
2. Store the new token in Bitwarden under `clix-mobile` folder.
...
2. Enable Sentry's "allowed domains" / ingest rate-limit: in Sentry project settings, set the allowed origins to `com.clix.mobile` (App ID) to block abuse from arbitrary origins. Android DSN abuse is harder to block; apply a Sentry ingest rate-limit rule.
...
**Remediation:**
1. Update the canonical privacy policy at `https://bolajiev.github.io/clix-mobile/privacy/` to explain that the app connects to user-configured server addresses that may use HTTP.
...
- **Certificate pinning for `bolajiev.com/opencode`:** Once the paid Cloud product launches, add cert pinning for the cloud endpoint using `react-native-ssl-pinning` or a custom `TrustKit` integration. Not needed while the app only connects to user-owned servers.
