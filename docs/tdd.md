# Clix Mobile — Technical Design Document (TDD)
...
- Sentry `release` is set to `clix-mobile@<version>` so source maps (uploaded by `sentry-cli` during build) line up with reported stack frames.
...
| Vercel website | `website/` | Versioned Next.js source deployed to existing `clix-mobile-site` project. |
