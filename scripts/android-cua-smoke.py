#!/usr/bin/env python3
"""
Android Computer-Use Agent (CUA) smoke test for Clix Mobile.
...
def run_scenario_sse_disconnect_banner(opencode_url: str, model: str, include_ui_xml: bool) -> dict:
    ...
    ok = run_cua_step(
        goal=(
            f"The app is open showing the Clix Mobile connections screen. "
...
def run_scenario_hello_world_e2e(
    ...
    model: str,
    include_ui_xml: bool,
    project_dir: str = "~/workspace/clix-mobile",
    ...
    ok = _phase(
        "open_project",
        goal=(
            "You are on the Sessions list screen of Clix Mobile. "
...
You are a mobile app test planner for an Android app called Clix Mobile.
...
def main():
    parser = argparse.ArgumentParser(
        description="Clix Mobile Android CUA smoke test — full onboarding showcase",
...
""",
    ...
    parser.add_argument(
        "--e2e-project-dir",
        default="~/workspace/clix-mobile",
        help="Project directory to open in the new session (default: ~/workspace/clix-mobile).",
    )
    parser.add_argument(
        "--e2e-model-hint",
        default="deepseek",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Example: 'Open app, connect to server, create session in ~/workspace/clix-mobile "
    ...
    if args.query:
        print("\n" + "=" * 64)
        print("  Clix Mobile — Query-Driven Test")
    ...
    if args.e2e:
        print("\n" + "=" * 64)
        print("  Clix Mobile — E2E Coding Task Scenario")
        print(f"  Server:   {connect_url}")
        print(f"  Model:    {args.model}")
    ...
    if not use_legacy:
        ...
        # ----------------------------------------------------------------
        print("\n" + "=" * 64)
        print("  Clix Mobile — Full Onboarding Showcase")
        print(f"  Server: {connect_url}")
        print(f"  Model:  {args.model}")
