...
export const defaultPreferences: Record<Category, boolean> = {
  // All categories ON by default — this is a tool for a solo user who WANTS
  // to know when the agent asks something, finishes, or the server drops.
  // Especially: permission requests + questions (must never be missed) and
  // task completion (the "I never know when you finished" complaint).
  permissions: true,
  questions: true,
  completed: true,
  errors: true,
...
export async function send(payload: Payload) {
  ...
  // Record the dedupe stamp only after a SUCCESSFUL schedule — otherwise a
  // failed send would suppress the retry for the whole cooldown.
  if (payload.dedupeKey) {
    dedupeSentAt.set(payload.dedupeKey, Date.now())
  }
}
