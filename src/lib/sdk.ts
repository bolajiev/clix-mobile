...
async function request<T>(
  ...
  const { response, cancel } = await requestWithTimeout(
    url,
    {
      ...options,
      headers,
...
async function fetchWithTimeout(url: string, options: RequestInit = {}, timeoutMs: number = REQUEST_TIMEOUT_MS): Promise<Response> {
  ...
  // NOTE: the timer is NOT cleared until the caller consumes the body —
  // request() calls response.json() after this resolves, and a server that
  // sends headers then stalls the body would otherwise hang forever. The
  // caller clears the timer via requestWithBody below.
  return new Promise<Response>((resolve, reject) => {
    fetch(url, { ...options, signal: controller.signal })
      .then((response) => {
        resolve(response)
        // Keep the abort wired for the body read.
      })
      .catch((error) => {
        if (timedOut) reject(new Error(`Request timed out after ${timeoutMs}ms`))
        else reject(error)
      })
  }).finally(() => {
    // Do NOT clearTimeout here — request() does it after json().
    parentSignal?.removeEventListener("abort", onParentAbort)
  })
}

// Body-aware variant: keeps the timeout armed until the body is read.
  ...
  return {
    global: {
      ...
      async *events(signal?: AbortSignal): AsyncGenerator<Event> {
        ...
        // Must use expo/fetch for ReadableStream support on native. The
        // connect has a hard timeout: a blackholed proxy that accepts the
        // TCP connection but never sends headers would otherwise hang the
        // reconnect loop forever ("connected" forever, zero retries).
        const connectController = new AbortController()
        const connectTimer = setTimeout(() => connectController.abort(), SSE_CONNECT_TIMEOUT_MS)
        const onSignalAbort = () => connectController.abort()
        signal?.addEventListener("abort", onSignalAbort)
        let response: Response
        try {
          response = await expoFetch(url, { headers, signal: connectController.signal })
        } finally {
        ...
        // Abort must cancel an in-flight body read, or a dropped stream
        // leaks a stuck generator (and every reconnect leaks another).
    ...
    session: {
      ...
      list: (params?: { roots?: boolean; limit?: number; search?: string }): Promise<Session[]> => {
        // Directory-scoped clients must list THAT project's sessions — the
        // server's default list only returns the global project, so sessions
        // created by CLI instances in other working directories (e.g. this
        // repo's own terminal conversations) would be invisible to the app.
        if (config.directory) {
          const query = `?directory=${encodeURIComponent(config.directory)}`
          return request<Session[]>(config, `/session${query}`)
        }
      ...
      // Status for ALL sessions in one call — the streaming safety net (D2):
      // polled while any session is busy so completion is never missed.
